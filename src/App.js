  ...
  const fetchData = async () => {
    ...
    await fetch("https://countriesnow.space/api/v0.1/countries")
      ...
      .then((result) => {
        ...
        console.log('hello', countriesAndCities)
        ...
        <div className="w-full h-screen flex justify-center items-center bg-[#F3F4F6]">
          ...
          <div className='fixed z-50 top-10'>
            <div className="w-[567px] h-auto bg-white flex row gap-4 justify-center items-center p-4 rounded-[48px]">
              ...
              <input type="search" id="search" name="search" placeholder="Search" onChange={handleSearch} className="h-[44px] w-full text-3xl font-bold outline-none"></input>
            </div>
            {countrySearch &&
              <div className='mt-[10px] pt-4 pb-4 bg-[#ffffff33] backdrop-blur-2xl shadow-lg rounded-[24px]'>
                {isLoading && <div>this is isLoading ...</div>}
                ...
                {!isLoading && countrySearch.length > 0 &&
                  filteredData.map((country, index) => {
                    ...
                    return <div className='w-full h-[50px] flex items-center self-stretch gap-[16px] cursor-pointer ml-[24px]'>
                      ...
                      <p className='text-2xl font-bold' key={index}>{country.city}, {country.country}</p>
                    </div>
                  })}
              </div>}
          </div>
          ...
          <div className="flex relative justify-center items-center">
            {/* Card */}
            <div className="w-[414px] h-[832px] flex flex-col items-center rounded-[42px] z-10 bg-white/20 backdrop-blur bg-opacity-20 shadow-lg">
              {/* Card Header f */}
            ...
            <div className='absolute w-[128px] h-[128px] rounded-full bg-gradient-to-r from-[#ff9429] via-[#feac31] to-[#ff9429] -ml-[400px] -mt-[60px] top-0 z-0'></div>
          </div>
        </div>
